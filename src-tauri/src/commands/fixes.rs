...
pub fn get_fixes(state: State<AppState>, search: Option<String>) -> Result<Vec<Fix>, String> {
    ...
    let mut stmt = db.prepare(
        "SELECT id,name,description,category,shell_type,command,tags,confirm_required FROM fixes ORDER BY category,name"
...
pub fn add_fix(state: State<AppState>, data: FixData) -> Result<i64, String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute(
        "INSERT INTO fixes (name,description,category,shell_type,command,tags,confirm_required) VALUES (?1,?2,?3,?4,?5,?6,?7)",
        params![data.name, data.description.unwrap_or_default(), data.category.unwrap_or_else(|| "General".into()),
                ...
                data.confirm_required.unwrap_or(false) as i64],
    ).map_err(|e| e.to_string())?;
...
pub fn update_fix(state: State<AppState>, id: i64, data: FixData) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute(
        "UPDATE fixes SET name=?1,description=?2,category=?3,shell_type=?4,command=?5,tags=?6,confirm_required=?7 WHERE id=?8",
        params![data.name, data.description.unwrap_or_default(), data.category.unwrap_or_else(|| "General".into()),
                data.shell_type.unwrap_or_else(|| "powershell".into()), data.command, data.tags.unwrap_or_default(),
                data.confirm_required.unwrap_or(false) as i64, id],
    ).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub async fn run_fix(app: tauri::AppHandle, state: State<'_, AppState>, id: i64) -> Result<RunResult, String> {
    let (command, shell_type, name) = {
        let db = state.0.lock().map_err(|e| e.to_string())?;
        db.query_row("SELECT command,shell_type,name FROM fixes WHERE id=?1", params![id], |row| {
            Ok((row.get::<_,String>(0)?, row.get::<_,String>(1)?, row.get::<_,String>(2)?))
        }).map_err(|e| e.to_string())?
    };
    let (program, args): (&str, Vec<String>) = match shell_type.as_str() {
        "powershell" => ("powershell", vec!["-ExecutionPolicy".into(), "Bypass".into(), "-Command".into(), command.clone()]),
