...
pub fn get_watchers(state: State<AppState>) -> Result<Vec<Watcher>, String> {
    ...
    let rows = stmt.query_map([], |r| Ok(Watcher {
        id: r.get(0)?, name: r.get(1)?, condition_type: r.get(2)?, condition_config: r.get(3)?,
        action: r.get(4)?, enabled: r.get::<_, i64>(5)? != 0,
        last_checked: r.get(6)?, last_state: r.get(7)?, last_triggered_at: r.get(8)?,
    })).map_err(|e| e.to_string())?;
...
pub fn delete_watcher(state: State<AppState>, id: i64) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("DELETE FROM watchers WHERE id=?1", params![id]).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn toggle_watcher(state: State<AppState>, id: i64, enabled: bool) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("UPDATE watchers SET enabled=?1 WHERE id=?2", params![enabled as i64, id]).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn start_watcher_scheduler(app: tauri::AppHandle) {
    std::thread::spawn(move || {
        ...
        loop {
            let app2 = app.clone();
            tauri::async_runtime::spawn(async move { check_watchers(&app2).await; });
...
async fn check_watchers(app: &tauri::AppHandle) {
    ...
    let needs_perf = rows.iter().any(|(_, ct, ..)| ct == "disk_below" || ct == "cpu_sustained");
    let perf = if needs_perf { crate::commands::misc::get_perf_stats(app.clone()).await.ok() } else { None };
...
fn eval_disk_below(perf: Option<&crate::commands::misc::PerfStats>, cfg: &serde_json::Value) -> bool {
    let Some(perf) = perf else { return false };
    let drive = cfg["drive"].as_str().unwrap_or("C");
    let pct = cfg["pct"].as_f64().unwrap_or(10.0);
    perf.drives.iter().find(|d| d.name.eq_ignore_ascii_case(drive)).is_some_and(|d| {
        if d.total_gb <= 0.0 { return false; }
        let free_pct = (d.total_gb - d.used_gb) / d.total_gb * 100.0;
        free_pct < pct
    })
}

fn eval_cpu_sustained(id: i64, perf: Option<&crate::commands::misc::PerfStats>, cfg: &serde_json::Value) -> bool {
    let Some(perf) = perf else { return false };
    ...
    let mut streaks = cpu_streaks().lock().unwrap();
...
async fn eval_process_down(app: &tauri::AppHandle, cfg: &serde_json::Value) -> bool {
    let name = cfg["process"].as_str().unwrap_or("").trim_end_matches(".exe").to_string();
    if name.is_empty() { return false; }
    let ps = format!("if (Get-Process -Name '{}' -ErrorAction SilentlyContinue) {{'1'}} else {{'0'}}", name.replace('\'', "''"));
...
async fn record_and_fire(app: &tauri::AppHandle, id: i64, alert: bool, action: &str, condition_type: &str, cfg: &serde_json::Value) {
    let new_state = if alert { "alert" } else { "ok" };
    let state = app.state::<AppState>();
    let was_alert: String = {
        let Ok(db) = state.0.lock() else { return };
        db.query_row("SELECT last_state FROM watchers WHERE id=?1", params![id], |r| r.get(0)).unwrap_or_else(|_| "ok".into())
    };

    if let Ok(db) = state.0.lock() {
        let _ = db.execute("UPDATE watchers SET last_checked=datetime('now'),last_state=?1 WHERE id=?2", params![new_state, id]);
    }

    // Only fire on the ok->alert transition — a persisting condition doesn't spam every 30s.
    if alert && was_alert != "alert" {
        if let Ok(db) = state.0.lock() {
            let _ = db.execute("UPDATE watchers SET last_triggered_at=datetime('now') WHERE id=?1", params![id]);
        }
        let msg = describe(condition_type, cfg);
        if let Some(wf_id) = action.strip_prefix("workflow:").and_then(|s| s.parse::<i64>().ok()) {
            let app2 = app.clone();
...
fn describe(condition_type: &str, cfg: &serde_json::Value) -> String {
    match condition_type {
        "disk_below" => format!("Drive {} is low on space", cfg["drive"].as_str().unwrap_or("?")),
        "process_down" => format!("{} is not running", cfg["process"].as_str().unwrap_or("?")),
        "cpu_sustained" => format!("CPU sustained above {}% for {} min", cfg["pct"].as_i64().unwrap_or(0), cfg["minutes"].as_i64().unwrap_or(0)),
