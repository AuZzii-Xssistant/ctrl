...
//! one PowerShell run: (1) snapshots the revertible bits, (2) applies every
//! enabled item, (3) records the profile as active. Restore replays the
//! snapshot in reverse.
//!
...
use crate::commands::exec::{Shell, run_elevated as exec_elevated};
...
const ITEM_TYPES: &[&str] = &["power_plan", "kill_apps", "start_apps", "dns", "audio", "refresh_rate", "script"];
...
fn load_items(db: &rusqlite::Connection, profile_id: i64) -> Result<Vec<ProfileItem>, String> {
    ...
    let rows = stmt.query_map(params![profile_id], |r| Ok(ProfileItem {
        id: r.get(0)?, item_type: r.get(1)?, value: r.get(2)?,
...
pub fn get_profiles(state: State<AppState>) -> Result<Vec<Profile>, String> {
    ...
    let mut stmt = db.prepare("SELECT id,name,description,icon FROM profiles ORDER BY name")
        .map_err(|e| e.to_string())?;
    ...
    for row in rows.filter_map(|r| r.ok()) {
        ...
        out.push(Profile { id, name, description, icon, items });
    }
    Ok(out)
}
...
pub fn add_profile(state: State<AppState>, data: ProfileData) -> Result<i64, String> {
    ...
    tx.execute(
        ...
        params![data.name, data.description.unwrap_or_default(), data.icon.unwrap_or_else(|| "ti-user-cog".into())],
    ).map_err(|e| e.to_string())?;
...
pub fn update_profile(state: State<AppState>, id: i64, data: ProfileData) -> Result<(), String> {
    ...
    tx.execute(
        ...
        params![data.name, data.description.unwrap_or_default(), data.icon.unwrap_or_else(|| "ti-user-cog".into()), id],
    ).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM profile_items WHERE profile_id=?1", params![id]).map_err(|e| e.to_string())?;
    insert_items(&tx, id, &data.items)?;
    tx.commit().map_err(|e| e.to_string())?;
    Ok(())
}

fn insert_items(tx: &rusqlite::Transaction, profile_id: i64, items: &[ProfileItemData]) -> Result<(), String> {
    for it in items {
        if !ITEM_TYPES.contains(&it.item_type.as_str()) { continue; }
        if it.value.trim().is_empty() { continue; }
        tx.execute(
            "INSERT INTO profile_items (profile_id,item_type,value,enabled) VALUES (?1,?2,?3,?4)",
            params![profile_id, it.item_type, it.value, it.enabled.unwrap_or(true) as i64],
        ).map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
pub fn delete_profile(state: State<AppState>, id: i64) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("DELETE FROM profiles WHERE id=?1", params![id]).map_err(|e| e.to_string())?;
    db.execute("UPDATE profile_state SET active_profile_id=NULL, active_since=NULL WHERE active_profile_id=?1", params![id]).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn get_active_profile(state: State<AppState>) -> Result<Option<ActiveProfile>, String> {
    ...
    ).map_err(|_| String::new()).map(Some).or(Ok(None))
}

fn esc_ps(s: &str) -> String { s.replace('\'', "''") }

/// Build the elevated PowerShell script for activation: snapshot markers
/// first (parsed back out of stdout after the run), then each enabled item.
fn build_activate_script(items: &[ProfileItem]) -> String {
    let mut s = String::from("[Console]::OutputEncoding=[Text.Encoding]::UTF8\n$ErrorActionPreference='Continue'\n");

    // --- snapshot (always captured, regardless of which items are enabled) ---
    s.push_str(
        ...
        match it.item_type.as_str() {
            ...
            "start_apps" => {
                for path in it.value.lines().map(str::trim).filter(|l| !l.is_empty()) {
                    s.push_str(&format!(
                        "try {{ $__p=Start-Process -FilePath '{p}' -PassThru -ErrorAction Stop; $__started += $__p.ProcessName }} catch {{ Write-Warning \"start failed: {p}\" }}\n",
                        p = esc_ps(path)
                    ));
                }
            }
            "dns" => {
                ...
                if v.eq_ignore_ascii_case("dhcp") {
                    ...
                    let ips: Vec<String> = v.split(',').map(|x| format!("'{}'", x.trim())).collect();
                    s.push_str(&format!(
...
'@\n\
  ...
  [CTRL.Disp]::ChangeDisplaySettings([ref]$__dm,0) | Out-Null\n\
    ...
    s.push_str("Write-Output (\"CTRL_SNAP:started=\" + ($__started -join ','))\n");
    s
}
...
fn build_restore_script(snap: &Snapshot) -> String {
    ...
    if !snap.dns_interface.is_empty() {
        if snap.dns_servers.is_empty() {
            ...
            let ips: Vec<String> = snap.dns_servers.split(',').map(|x| format!("'{}'", esc_ps(x.trim()))).collect();
    ...
    for name in snap.started_apps.split(',').map(str::trim).filter(|l| !l.is_empty()) {
...
fn parse_snapshot(output: &str) -> Snapshot {
    let mut snap = Snapshot { power_plan: String::new(), dns_interface: String::new(), dns_servers: String::new(), audio_device: String::new(), started_apps: String::new() };
    for line in output.lines() {
        let Some(rest) = line.trim().strip_prefix("CTRL_SNAP:") else { continue };
...
#[tauri::command]
pub async fn activate_profile(app: tauri::AppHandle, state: State<'_, AppState>, id: i64) -> Result<RunResult, String> {
    let (name, items) = {
        let db = state.0.lock().map_err(|e| e.to_string())?;
        let name: String = db.query_row("SELECT name FROM profiles WHERE id=?1", params![id], |r| r.get(0))
            .map_err(|_| "Profile not found".to_string())?;
        (name, load_items(&db, id)?)
    };

    let script = build_activate_script(&items);
    let result = exec_elevated(&app, &script, &Shell::PowerShell, "profile").await?;
    let snap = parse_snapshot(&result.output);
...
#[tauri::command]
pub async fn restore_previous(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<RunResult, String> {
    let (profile_id, snap) = {
        let db = state.0.lock().map_err(|e| e.to_string())?;
        let profile_id: Option<i64> = db.query_row("SELECT active_profile_id FROM profile_state WHERE id=1", [], |r| r.get(0)).ok().flatten();
        let Some(profile_id) = profile_id else {
            return Ok(RunResult { success: false, output: "No active profile to restore from.".into() });
        };
    ...
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("UPDATE profile_state SET active_profile_id=NULL, active_since=NULL WHERE id=1", []).map_err(|e| e.to_string())?;
    drop(db);
    let _ = profile_id;
