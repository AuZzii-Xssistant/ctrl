...
pub fn get_backup_jobs(state: State<AppState>) -> Result<Vec<BackupJob>, String> {
    ...
    let mut stmt = db.prepare(
        "SELECT id,name,source,dest,last_run,created_at FROM backup_jobs ORDER BY name"
    ).map_err(|e| e.to_string())?;
    let rows = stmt.query_map([], |row| {
        Ok(BackupJob { id: row.get(0)?, name: row.get(1)?, source: row.get(2)?,
                       dest: row.get(3)?, last_run: row.get(4)?, created_at: row.get(5)? })
    }).map_err(|e| e.to_string())?;
...
pub fn add_backup_job(state: State<AppState>, data: BackupData) -> Result<i64, String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("INSERT INTO backup_jobs (name,source,dest) VALUES (?1,?2,?3)",
        params![data.name, data.source, data.dest]).map_err(|e| e.to_string())?;
...
pub fn update_backup_job(state: State<AppState>, id: i64, data: BackupData) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("UPDATE backup_jobs SET name=?1,source=?2,dest=?3 WHERE id=?4",
        params![data.name, data.source, data.dest, id]).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn delete_backup_job(state: State<AppState>, id: i64) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("DELETE FROM backup_jobs WHERE id=?1", params![id]).map_err(|e| e.to_string())?;
    Ok(())
}

#[tauri::command]
pub async fn run_backup(app: tauri::AppHandle, state: State<'_, AppState>, id: i64) -> Result<RunResult, String> {
    let (source, dest, name): (String, String, String) = {
        let db = state.0.lock().map_err(|e| e.to_string())?;
        db.query_row("SELECT source,dest,name FROM backup_jobs WHERE id=?1", params![id],
            |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?))).map_err(|e| e.to_string())?
    };
    // robocopy exit codes 0-7 are success (8+ are errors)
    let out = app.shell().command("robocopy")
