...
#[tauri::command]
pub fn get_projects(state: State<AppState>, search: Option<String>) -> Result<Vec<Project>, String> {
    ...
    let rows = stmt.query_map([], |row| {
        Ok(Project {
            ...
            r#type: row.get(3)?, status: row.get(4)?, path: row.get(5)?,
            tags: row.get(6)?, notes: row.get(7)?,
        })
    }).map_err(|e| e.to_string())?;
    Ok(rows.filter_map(|r| r.ok())
        .filter(|p| q.is_empty() || p.name.to_lowercase().contains(&q) || p.tags.to_lowercase().contains(&q))
        .collect())
}
...
#[tauri::command]
pub async fn open_project_path(app: tauri::AppHandle, state: State<'_, AppState>, id: i64) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    let path: String = db.query_row("SELECT path FROM projects WHERE id=?1", params![id], |r| r.get(0)).map_err(|e| e.to_string())?;
    drop(db);
    if !path.is_empty() {
