...
pub fn get_workflows(state: State<AppState>) -> Result<Vec<Workflow>, String> {
    ...
    let rows = stmt.query_map([], |r| Ok(Workflow {
        id: r.get(0)?, name: r.get(1)?, description: r.get(2)?, steps: r.get(3)?,
        enabled: r.get::<_,i64>(4)? != 0,
        trigger_type: r.get(5)?, trigger_config: r.get(6)?,
        last_run_at: r.get(7)?, last_run_ok: r.get::<_,Option<i64>>(8)?.map(|v| v != 0),
        created_at: r.get(9)?,
    })).map_err(|e| e.to_string())?;
...
pub fn toggle_workflow(state: State<AppState>, id: i64, enabled: bool) -> Result<(), String> {
    let db = state.0.lock().map_err(|e| e.to_string())?;
    db.execute("UPDATE workflows SET enabled=?1 WHERE id=?2", params![enabled as i64, id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
pub async fn run_workflow(app: tauri::AppHandle, state: State<'_, AppState>, id: i64) -> Result<Vec<StepResult>, String> {
    let (steps_json, wf_name): (String, String) = {
        let db = state.0.lock().map_err(|e| e.to_string())?;
        db.query_row("SELECT steps,name FROM workflows WHERE id=?1", params![id],
    ...
    let _ = app.emit("wf-step", serde_json::json!({ "wf_id": id, "step": 0, "total": steps.len() }));
    ...
    for (i, step) in steps.iter().enumerate() {
        ...
        results.push(StepResult { label: step.label.clone(), success, output });
    }
...
async fn run_step_script(app: &tauri::AppHandle, state: &State<'_, AppState>, step: &Step) -> Result<(bool, String), String> {
    ...
    let out = app.shell().command(prog).args(&args).output().await.map_err(|e| e.to_string())?;
    let text = String::from_utf8_lossy(&out.stdout).to_string() + &String::from_utf8_lossy(&out.stderr);
    Ok((out.status.success(), text))
}

async fn run_step_fix(app: &tauri::AppHandle, state: &State<'_, AppState>, step: &Step) -> Result<(bool, String), String> {
    let id = step.item_id.ok_or("missing item_id")?;
    let (cmd, st): (String, String) = {
        let db = state.0.lock().map_err(|e| e.to_string())?;
        db.query_row("SELECT command,shell_type FROM fixes WHERE id=?1", params![id],
            |r| Ok((r.get(0)?, r.get(1)?))
        ).map_err(|e| e.to_string())?
    };
    let (prog, args): (&str, Vec<String>) = match st.as_str() {
        "powershell" => ("powershell", vec!["-ExecutionPolicy".into(), "Bypass".into(), "-Command".into(), cmd]),
...
fn fire_matching(app: &tauri::AppHandle, trigger_type: &str, hhmm: Option<&str>) {
    ...
    let ids: Vec<(i64, String)> = {
        ...
        let Ok(mut stmt) = db.prepare(
            "SELECT id,trigger_config FROM workflows WHERE enabled=1 AND trigger_type=?1"
        ) else { return };
        stmt.query_map(params![trigger_type], |r| Ok((r.get::<_,i64>(0)?, r.get::<_,String>(1)?)))
...
fn check_days(config: &serde_json::Value) -> bool {
    let Some(days) = config["days"].as_array() else { return true };
    if days.is_empty() { return true; }
