...
pub fn get_builder_actions(_app: tauri::AppHandle) -> Result<BuilderDefs, String> {
    // Check next to exe first (portable), then cwd (dev)
    let exe_dir = std::env::current_exe().ok()
        .and_then(|p| p.parent().map(|d| d.to_path_buf()))
        .unwrap_or_else(|| std::path::PathBuf::from("."));
    let data_dir = if exe_dir.join("data").join("builder").exists() {
        exe_dir.join("data").join("builder")
    } else {
        std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."))
            .join("data").join("builder")
    };
...
pub fn save_built_script(state: State<AppState>, code: String, name: String, script_type: String) -> Result<(), String> {
    ...
    let tmp = std::env::temp_dir().join(format!("{}.{}", name.replace(' ', "_"), script_type));
    std::fs::write(&tmp, &code).map_err(|e| e.to_string())?;
    db.execute(
        "INSERT INTO scripts (name,description,category,file_path,script_type) VALUES (?1,'Generated by Builder','Builder',?2,?3)",
        params![name, tmp.to_string_lossy().to_string(), script_type],
