...
- **Single-instance guard**: added `tauri-plugin-single-instance`, registered first in the plugin chain. Launching `ctrl.exe` a second time now just focuses the existing window instead of opening a second process — closes off a real gap where a second instance's startup temp-cleanup could delete a first instance's active elevated-run files mid-run, plus the double-open-same-`ctrl.db` risk that created.
- **Orphaned processes on exit**: `_quitMaybeConfirm()` now iterates every open terminal tab and calls `pty_close` for each, and kills `_elevatedPid` if a "Run As Admin" console is active, before calling `exit_app`. Neither `portable-pty` nor a UAC-elevated external console dies automatically when the parent process exits on Windows — previously both could survive as invisible background processes after CTRL fully closed.
- **Closing a tab (×) while it's running**: `_closeTab()` now checks `runLock`/`shellBusy` for that specific tab and confirms before killing it, mirroring the guard the app-quit path already had. Same failure mode as the earlier "quit force-kills a running script" fix, just reachable through the tab-close button instead.
- **Run As Admin now captures real output into History**: `run_elevated` (`exec.rs`) wraps the elevated script with `Start-Transcript`/`Stop-Transcript` — passively records the console into a temp file without changing what runs or how it renders in the external elevated window. The Rust-side poll thread reads and cleans that transcript once the run finishes and returns it as the real `RunResult.output`, which flows through the existing `run_log` insert Scripts/Fixes already do — so it now shows up in the output modal and History, never written into the live embedded terminal. This was a direct redirect from an earlier proposed pipe/log-tail approach the user flagged as wrong.
- **`builder.js` no longer crashes on corrupted `localStorage`**: `_appsSel`'s initial `JSON.parse` (module-load time) is now wrapped in the same try/catch-empty-Set pattern `tweaks.js` already used for the identical read.
- **ScriptStash import now tells you when the file's the wrong format**: `ss_import_profile` returns a real `Err` when the picked JSON has no `"scripts"` array, instead of silently succeeding with "Imported 0 script(s)" — indistinguishable before from a genuinely empty (but valid) export.
- **SQLite `busy_timeout` set on both connections**: `PRAGMA busy_timeout=5000` (GUI) and `Connection::busy_timeout()` (CLI) — a concurrent write from `ctrl-cli.exe` while the GUI is mid-write no longer fails outright with `SQLITE_BUSY`, it waits briefly instead. WAL mode already handled reader/writer conflicts; this covers the remaining writer-vs-writer case.
